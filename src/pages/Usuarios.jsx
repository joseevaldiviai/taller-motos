...
const s = {
  ...
  tableHead: { display:'grid', gridTemplateColumns:'1.5fr 1fr 90px 130px 120px', padding:'10px 16px', fontSize:11, fontWeight:700, letterSpacing:'.8px', textTransform:'uppercase', color:'var(--text-muted)', borderBottom:'1px solid var(--border)', background:'var(--bg)' },
  tableRow:  { display:'grid', gridTemplateColumns:'1.5fr 1fr 90px 130px 120px', padding:'13px 16px', alignItems:'center', borderBottom:'1px solid var(--divider)', fontSize:13 },
  badge:     (c) => ({ display:'inline-block', padding:'3px 10px', borderRadius:9999, fontSize:11, fontWeight:700, background:`${c}22`, color:c }),
  nameCell:  { display:'flex', alignItems:'center' },
  avatar:    (c) => ({ width:32, height:32, borderRadius:'50%', background:c, display:'inline-flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:'bold', color:'var(--accent-contrast)', marginRight:8, flexShrink:0 }),
  actBtns:   { display:'flex', gap:6 },
  iconBtn:   (c) => ({ padding:'5px 8px', borderRadius:6, border:`1px solid ${c}44`, background:`${c}11`, color:c, cursor:'pointer', display:'flex', alignItems:'center' }),
  ...
  modal:     { width:'100%', maxWidth:460, background:'var(--card)', border:'1px solid var(--border)', borderRadius:14, padding:28, maxHeight:'90vh', overflowY:'auto' },
...
function ModalUsuario({ token, usuario, onClose, onSaved }) {
  ...
  const [form, setForm] = useState({
    ...
    sueldo_semanal: usuario?.sueldo_semanal || usuario?.sueldo_base || '',
    password:     '',
  })
  ...
  const handleSave = async (e) => {
    ...
    if (!isEdit && form.password.length < 6) { toast.error('Mínimo 6 caracteres'); return }
    setSaving(true)
    const data = {
      nombre:        form.nombre,
      ...
      sueldo_semanal: form.rol === 'MECANICO' && form.sueldo_semanal ? parseFloat(form.sueldo_semanal) : null,
        ...
        <div style={s.modal}>
          ...
          <form onSubmit={handleSave}>
            ...
            <input style={s.input} value={form.nombre}
                   onChange={e=>setForm(f=>({...f,nombre:e.target.value}))} required placeholder="Juan Pérez"/>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
              <div>
                ...
                <input style={s.input} value={form.username}
                       ...
                       required placeholder="juanp" disabled={isEdit}/>
              </div>
              <div>
                <label style={s.label}>Email *</label>
                <input style={s.input} type="email" value={form.email}
                       onChange={e=>setForm(f=>({...f,email:e.target.value}))}
                       required placeholder="juan@taller.com" disabled={isEdit}/>
              </div>
            </div>
            ...
            {form.rol === 'MECANICO' && (
                <>
                  ...
                  <select style={s.select} value={form.tipo_sueldo}
                          onChange={e=>setForm(f=>({...f,tipo_sueldo:e.target.value}))}>
                    <option value="FIJO">Fijo semanal</option>
                    <option value="COMISION">Comisión (% sobre servicios realizados)</option>
                  </select>

                  {form.tipo_sueldo === 'FIJO' && (
                      <>
                        <label style={s.label}>Sueldo semanal (Bs.) *</label>
                  ...
                  {form.tipo_sueldo === 'COMISION' && (
                      <>
                        <label style={s.label}>% de comisión sobre servicios realizados</label>
                        <input style={s.input} type="number" min="0" max="100" step="0.1"
                               value={form.sueldo_semanal}
                               onChange={e=>setForm(f=>({...f,sueldo_semanal:e.target.value}))}
                               placeholder="Ej: 30 (30%)"/>
                        <div style={s.infoBox}>
                          La comisión se calcula solo sobre los servicios realizados por el mecánico —
                          no incluye reemplazos ni trabajos externos.
                        </div>
                      </>
                  )}
                </>
      ...
      <div style={s.shell}>
        ...
        <div style={{display:'flex',gap:8,marginBottom:16,flexWrap:'wrap'}}>
          {['TODOS','ADMIN','MECANICO'].map(r=>(
              <button key={r} onClick={()=>setFiltroRol(r)} style={{
                ...
                background: filtroRol===r?'var(--accent)':'transparent',
                color: filtroRol===r?'var(--accent-contrast)':'var(--text-soft)',
                fontSize:12, fontWeight: filtroRol===r?700:400, cursor:'pointer'
              }}>
                {r==='TODOS'?'Todos':r}
              </button>
          ))}
        ...
        <div style={s.card}>
          <div style={s.tableHead}>
            <span>Nombre</span><span>Usuario</span><span>Rol</span><span>Sueldo</span><span>Acciones</span>
          </div>
          {loading
                  ...
                  : filtrados.map(u=>(
                      <div key={u.id} style={{...s.tableRow, opacity:u.activo?1:.5}}>
                        ...
                        <div>
                          {u.rol==='MECANICO' && u.tipo_sueldo
                              ? <div>
                                ...
                                {(u.sueldo_semanal || u.sueldo_base) && (
                                    <div style={{fontSize:11,color:'var(--text-muted)',marginTop:3}}>
                                      {u.tipo_sueldo==='FIJO'
                                          ? `Bs. ${u.sueldo_semanal || u.sueldo_base}/semana`
                                          : `${u.sueldo_semanal || u.sueldo_base}% de servicios`
                                      }
                                    </div>
                                )}
                              </div>
                        ...
                        <div style={s.actBtns}>
                          ...
                          <button style={s.iconBtn(u.activo?'var(--danger)':'var(--success)')}
        ...
        {(modal==='crear'||modal?.usuario) && (
            <ModalUsuario token={token} usuario={modal?.usuario||null}
